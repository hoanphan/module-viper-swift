...
//            let date = calendar.date(byAdding: .hour, value: Int(localTimeZoneAbbreviation!)!, to: self)
...
//        }
   ...
   static func  compareDate(date: Date, currentDate: Date) -> Int? {
       return Calendar.current.dateComponents([.day], from: date, to: currentDate).day
    }
}
